...
for current_mkt in all_mkt:
    ...
    config['save_path'] = f'./checkpoint/Bert4XMR_{current_mkt}_pretrain.pt'
    ...
    loader_generator = Bert4XMR_loader(train, valid, test, config)
    train_loader = loader_generator.get_loader('train')


    model = Bert4XMR(config)
    ...
    for epoch in range(config['num_epoch']):
        ...
        for data in train_loader:
            ...
            loss.backward()
            optimizer.step()
        result = eval_model_Bert4XMR(model, valid, config, 'valid', loader_generator)
    ...
    res = eval_model_Bert4XMR(model, test, config, 'test', loader_generator)
    ...
    for epoch in range(config['num_epoch']):
        ...
        print('-'*90)
    model = load_model(model, config)
    res = eval_model_Bert4XMR(model, test, config, 'test', loader_generator, cold_start=cold_start)
